...
if __name__ == "__main__":
    env = TFEnvironmentBaseWrapper(
        suite_gym.load("Sips-v0", max_episode_steps=MAX_STEPS)
    )
    ...
    actor_net = actor_distribution_network.ActorDistributionNetwork(
        obs_spec, act_spec, fc_layer_params=[226, 100, 50, 3]
    )
    ...
    driver = DynamicEpisodeDriver(
        env, agent.collect_policy, observers=[replay_buffer.add_batch], num_episodes=1
    )
