...
def remove_missed_wins(games):
    ...
    if isinstance(games, dict):
        for g_id in list(games.keys()):
            if (
                len(games[g_id]["a_win"].unique()) + len(games[g_id]["h_win"].unique())
                != 3
            ):
                del games[g_id]
...
def dates(df):
    ...
    df["datetime"] = pd.to_datetime(
        df["datetime"], infer_datetime_format=True, errors="coerce"
    )
