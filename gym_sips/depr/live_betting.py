...
def main():
    ...
    sip = ll.Sippy(
        fn=None, header=0, league="mlb", verbosity=0
    )  # start scraper for nba, not writing to file
    ...
    try:
        while True:
            ...
            for game in sip.games:
                ...
                a = dqn.choose_action(
                    cur_state
                )  # give deep q network state and return action
                next_state, r, d, odds = env.step(
                    a, cur_state
                )  # next state, reward, done, odds
